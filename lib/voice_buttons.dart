...
class _VoiceButtonsState extends State<VoiceButtons> {
  ...
  void setSDConfig() async {
    ...
    // Always keep the clients dir that some distros expect
    Directory(path.join(confDir.path, "clients")).create();
  
    // --- speechd.conf: back up then replace with Pied template ---
    ...
    if (sdConfig.existsSync()) {
      try {
        ...
        sdBackup.writeAsBytesSync(sdConfig.readAsBytesSync());
      } catch (_) {}
      try {
        sdConfig.deleteSync();
      } catch (_) {}
    }
    sdConfig.createSync(recursive: true);
    sdConfig.writeAsString(sdConfigTemplate);
  
    // --- modules/piper.conf: back up then replace with Pied template
    ...
    if (piperConf.existsSync()) {
      try {
        piperBackup.writeAsBytesSync(piperConf.readAsBytesSync());
      } catch (_) {}
      try {
        piperConf.deleteSync();
      } catch (_) {}
    }
    ...
    Timer(const Duration(seconds: 2), () {
      confirmConfigChange(null);
    });
  }

  Future<void> revertConfig(Directory? originalConfigDir) async {
    // Restore from file-level backups instead of replacing the whole directory.
    Directory confDir =
        Directory(path.join(getHome()!, ".config/speech-dispatcher"));
    Directory modulesDir = Directory(path.join(confDir.path, "modules"));
    if (!confDir.existsSync()) {
    ...
    // make sure backups are present
    if (sdBackup.existsSync()) {
      // Remove current configs
      if (sdConfig.existsSync()) {
      try { sdConfig.deleteSync(); } catch (_) {}
      }
      // Restore from backups
      try { sdBackup.copySync(sdConfig.path); } catch (_) {}
    }
    // make sure backups are present
    if (piperBackup.existsSync()) {
      // Remove current configs
      if (piperConf.existsSync()) {
      try { piperConf.deleteSync(); } catch (_) {}
      }
      // Restore from backups
      try { piperBackup.copySync(piperConf.path); } catch (_) {}
    }
